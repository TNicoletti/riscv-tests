...
void load_init_values_vector(int* vet, int regs[NUM_REGISTERS]){    
    ...
    for(int i = 0; i < NUM_REGISTERS; i++)
        load_to_vet(&vet[regs[i] * VLEN / SEW], regs[i]);
}

void store_vet_values(int r[NUM_REGISTERS], int* vet_res, int el_per_block, int num_registers){
    //clean_vector_scalar(&vet_res[0], el_per_block * num_registers);

    set_vet_settings();
    for(int i = 0; i < num_registers; i++)
        store_to_vet(&vet_res[i * el_per_block], r[i]);
}
...
int add_instruction(int op, int rxa[3], int r[3]){
    ...
    switch (op){
        ...
        case VMV_V_X:
            ...
            break;
        case VWADD_VV:
            if(widening_forbid(rx)) return -1;
...
int compare_solutions(int prev_error, int r[3], int* vet_res){
    ...
    for(int i = 0; i < 3; i++){
        if(!manual_convergence(&scalar_res[r[i]][0], &vet_res[r[i] * EL_PER_BLOCK], 1, EL_PER_BLOCK)){
...
void execute_RIS(int* vet, int r[NUM_REGISTERS], int32_t address_vector[], int* vet_res){
    ...
    store_vet_values(r, vet_res, EL_PER_BLOCK, NUM_REGISTERS);
}