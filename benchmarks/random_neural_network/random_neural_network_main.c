...
volatile int32_t scalar_res[3][4];
volatile int32_t vet_res[3][4];
...
void shuffle_registers(int r[3]){
    ...
    if(PRINTS) printf("Register choice:\nr[0]= %d; r[1]= %d; r[2]= %d;\n\n", r[0], r[1], r[2]);
}
...
store_vet_values(int r[3]){
    ...
    clean_matrix(vet_res, 3, 4);
    print_matrix(vet_res, 3, 4);
...
void error_discoverer(int index){
    ...
    for(int i = 0; i < 5; i++){
        ...
        if(is_divergent_matrix(scalar_res, vet_res, 3, 4)){
            printf("Convergence\n");
            passed[0]++;
        }else{
            printf("Divergence\n");
    ...
    for(int z = 0; z + inc < N; z+= inc){
        ...
        if(is_divergent_matrix(scalar_res, vet_res, 3, 4)){
            ...
            if(PRINTS >= 3){
                ...
                print_vector(&OUT[z], inc, (VLEN / SEW));
                printf("\n");
            }
            if(PRINTS){    
