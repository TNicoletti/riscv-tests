...
extern int vector_agnostic_sum_1(int* vector,int LENGTH);

extern int vector_unchanged_sum_1(int* vector,int LENGTH);
...
int agnostic_trend_test()
{
    ...
    vector_agnostic_sum_1(vector_test, COMPUTED_LENGTH);

    init_vector_seq(vector_compare, TRUE_LENGTH, 51);
    int res = compare_vectors(vector_test, vector_compare, COMPUTED_LENGTH);

    if(res){
        printf("AGNOSTIC test fail\n");
        return 1;
    }

    int fa = vector_test[1022];
    int sa = vector_test[1023];
    if(fa == vector_compare[1022])
        printf("VECTOR AGNOSTIC computed(1)\n");
    else if(fa == (1022 + 50))
        printf("VECTOR AGNOSTIC unchanged\n");
    else
        printf("VECTOR AGNOSTIC changed to random value(%d)\n", fa);
    if(sa == vector_compare[1023])
        printf("VECTOR AGNOSTIC computed(1)\n");
    else if(sa == (1023 + 50))
        printf("VECTOR AGNOSTIC unchanged\n");
    else
...
int unchanged_test()
{
    ...
    int sa = vector_test[1023];
    if(res){
        printf("UNCHANGED test fail\n");
        return 1;
    }

    if(fa != (1022 + 50)){
        printf("value 1022 should remain%d and became %d\n", (1022 + 50 - 2), fa);
        return 1;
    }
    if(sa != (1023 + 50)){
        printf("value 1023 should remain%d and became %d\n", (1023 + 50 - 2), sa);
        return 1;
    }

    printf("UNCHANGED TEST SUCCEED\n");
